...
class DependencyProvider implements DependencyProviderInterface
{
    ...
    public function defineDependencies(Locator $locator)
    {
        $locator
            ->Dependency()
            ->getFacade()
            ->setDependency(Module::COMMAND_COLLECTION_DEPENDENCY, function(){
        ...
        $locator
            ->Dependency()
            ->getFacade()
            ->setDependency(Module::APPLICATION_DEPENDENCY, function() use($locator){
                /* @var CommandCollection $commandCollection */
                $commandCollection = $locator->Dependency()->getFacade()->getDependency(Module::COMMAND_COLLECTION_DEPENDENCY);
